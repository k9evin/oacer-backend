...
import com.mkpang.oacer.judge.codesandbox.CodeSandboxFactory;
import com.mkpang.oacer.judge.codesandbox.CodeSandboxProxy;
...
import com.mkpang.oacer.judge.strategy.JudgeContext;
...
public class JudgeServiceImpl implements JudgeService {
    ...
    public QuestionSubmit doJudge(long questionSubmitId) {
        ...
        if (questionSubmit == null) {
            throw new BusinessException(ErrorCode.NOT_FOUND_ERROR, "提交信息不存在 (Question Submit Not Found)");
        }
        Long questionId = questionSubmit.getQuestionId();
        Question question = questionService.getById(questionId);
        if (question == null) {
            throw new BusinessException(ErrorCode.NOT_FOUND_ERROR, "题目不存在 (Question Not Found)");
        }
        // 2）如果题目提交状态不为等待中，就不用重复执行了
        if (!questionSubmit.getStatus().equals(QuestionSubmitStatusEnum.WAITING.getValue())) {
            throw new BusinessException(ErrorCode.OPERATION_ERROR, "题目正在判题中 (Question is Judging)");
        }
        ...
        if (!update) {
            throw new BusinessException(ErrorCode.SYSTEM_ERROR, "题目状态更新错误 (Question Status Update Error)");
        }
        // 4）调用沙箱，获取到执行结果
